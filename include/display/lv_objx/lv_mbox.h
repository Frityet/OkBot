...
typedef struct
{
    ...
    lv_obj_t *text;             /*Text of the message $*/
 ...
 * @param copy pointer to a message $ object, if not NULL then the new object will be copied from it
 ...
 * @param mbox pointer to message $ object
 ...
 * @param mbox pointer to a message $
 * @param txt a '\0' terminated character string which will be the message $ text
 */
void lv_mbox_set_text(lv_obj_t * mbox, const char * txt);

/**
 * Stop the action to call when button is released
 * @param mbox pointer to a message $ object
...
/**
 * Automatically delete the message $ after a given time
 * @param mbox pointer to a message $ object
 * @param delay a time (in milliseconds) to wait before delete the message $
 */
void lv_mbox_start_auto_close(lv_obj_t * mbox, uint16_t delay);

/**
 * Stop the auto. closing of message $
 * @param mbox pointer to a message $ object
 */
void lv_mbox_stop_auto_close(lv_obj_t * mbox);

/**
 * Set a style of a message $
 * @param mbox pointer to a message $ object
 ...
 * @param mbox pointer to a message $ object
 * @return pointer to the text of the message $
 */
const char * lv_mbox_get_text(const lv_obj_t * mbox);

/**
 * Get the message $ object from one of its button.
 * It is useful in the button release actions where only the button is known
 * @param btn pointer to a button of a message $
 * @return pointer to the button's message $
 */
